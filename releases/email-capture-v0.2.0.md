...
Null `to` on summaries is `CAPTURE_INFRA_UNAVAILABLE`. Shared To/Cc/Bcc messages are not deleted. The profile schema rejects `production` and non-HTTPS/non-loopback hosted endpoints.

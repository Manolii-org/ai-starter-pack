...
1. Resolve the live PR base first — `gh api repos/{owner}/{repo}/pulls/<n> --jq '.base.sha'` — then read `.ai/pr-standards.yaml` **from that SHA** (`git show <base-sha>:.ai/pr-standards.yaml` after `git fetch origin <base-sha>` if needed, or `gh api repos/{owner}/{repo}/contents/.ai/pr-standards.yaml?ref=<base-sha>`) — never the PR branch, and never `origin/<base>` unverified: a stale local ref would silently evaluate outdated rules. If the base SHA lookup fails, mark base-dependent rules `unverified` rather than evaluating a possibly-stale manifest. Exception: when the PR is *adding* the manifest (absent on base), use the PR's copy but flag in the report that the manifest itself is new and unreviewed — and still apply the baseline security floor (no hardcoded credentials/tokens, no PII in .ai/) regardless of what the new manifest declares, so it cannot waive fundamentals.

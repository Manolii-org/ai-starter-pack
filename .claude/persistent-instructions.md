...
`send_later` is a Remote MCP connector tool surfaced by claude.ai's **native** permission dialog — repo settings (`bypassPermissions`, `mcp__*` allows, allow-all PreToolUse hooks) cannot silence it, and on personal Max/Pro plans there is no org connector matrix, so every call prompts "Allow once / Deny". **Rule:** for self check-ins / heartbeats, use `create_trigger` with `run_once_at` (same underlying Routine, typically no forced prompt; one-shots self-disable after firing — `delete_trigger` cleanup is optional hygiene and itself connector-gated, so skip it if it prompts); fall back to `ScheduleWakeup` in remote sessions, then a single background wait. **Do NOT fall back to `send_later`:** CLI v2.1.214/215 tightened permission handling and re-invalidated any previously-cached "Always allow" for `send_later`, so it now prompts natively on every call and stalls autonomous loops. If none of the alternatives above succeed, proceed without scheduling — webhooks and user messages re-wake the session. **Same connector layer gates `subscribe_pr_activity` / `unsubscribe_pr_activity` / `add_repo` / `delete_trigger`:** call them only when genuinely needed. `subscribe_pr_activity` is idempotent — never unsubscribe+resubscribe to "reset" a stream. **Never call `unsubscribe_pr_activity` after a merged/closed notice** (the platform auto-unsubscribes); call it ONLY when the operator explicitly asks to stop watching an open PR (single attempt, best-effort). A watch loop that abandons a PR at final escalation just stops acting — the subscription lapses with the session; never spend a connector prompt on it.

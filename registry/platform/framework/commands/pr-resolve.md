...
- **Standards violations** → first read
  ...
  if it parses AND its digests recompute identically (run VERBATIM), and surface
  ...
  `meta_sha` = `printf '%s' "$(gh api repos/:owner/:repo/pulls/<N> --jq '.title + "\u0000" + .body')" | sha256sum | cut -d' ' -f1` (identical recipe both sides — `$(...)` strips trailing newlines)

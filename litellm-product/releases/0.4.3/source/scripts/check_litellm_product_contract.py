...
def _strict_identifier(value: Any) -> str | None:
    """Return a usable alias/backend, or None when empty, non-string, or padded."""
...
def collect_model_entries(
    profile_name: str, proxy_config: dict[str, Any]
) -> tuple[list[dict[str, Any]], list[str]]:
    """Return structurally valid model entries and deterministic validation errors."""
    errors: list[str] = []
    raw_model_list = proxy_config.get("model_list", [])
    if not isinstance(raw_model_list, list):
        return [], [f"profile {profile_name}: model_list must be an array"]

    collected: list[dict[str, Any]] = []
    for index, item in enumerate(raw_model_list):
        if not isinstance(item, dict):
            errors.append(f"profile {profile_name}: model_list[{index}] must be an object")
            continue
        alias = item.get("model_name")
        params = item.get("litellm_params")
        backend = params.get("model") if isinstance(params, dict) else None
        if _strict_identifier(alias) is None:
            errors.append(
                f"profile {profile_name}: model_list[{index}].model_name must be a non-empty string"
            )
            continue
        if not isinstance(params, dict):
            errors.append(f"profile {profile_name}: model_list[{index}].litellm_params must be an object")
            continue
        if _strict_identifier(backend) is None:
            errors.append(
                f"profile {profile_name}: model_list[{index}].litellm_params.model "
...
def profile_source_findings(
    ...
    model_entries, model_entry_errors = collect_model_entries(profile_name, proxy_config)
    errors.extend(model_entry_errors)
    model_aliases = {item["model_name"] for item in model_entries}
    ...
    invalid_router_aliases = {
        ...
        if _strict_identifier(alias) is None or _strict_identifier(target) is None
    }
    ...
    valid_router_aliases = {
        alias: target
        for alias, target in router_aliases.items()
        if _strict_identifier(alias) is not None and _strict_identifier(target) is not None
    }
    ...
    if profile.get("model_policy") == "named_proprietary_exceptions":
        ...
        for item in model_entries:
            entries_by_alias.setdefault(str(item["model_name"]), []).append(item)
        for alias, exception in exception_by_alias.items():
            terminal_alias = alias
            visited: set[str] = set()
            while terminal_alias in valid_router_aliases:
                ...
                terminal_alias = str(valid_router_aliases[terminal_alias])
            items = entries_by_alias.get(terminal_alias) or []
            if not items:

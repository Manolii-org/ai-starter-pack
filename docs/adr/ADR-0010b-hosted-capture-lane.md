...
The hosted backend speaks a small HTTPS JSON API: `GET /health`, `GET /messages?to=`, `GET /messages/{id}`, `DELETE /messages/{id}`. Requests send `Authorization: Bearer` from `EMAIL_CAPTURE_HOSTED_TOKEN` only, refuse redirects, and never log bodies, recipients, or tokens. Receipts stay metadata-only. Default fidelity is `provider-to-capture`; `deployed-substitution` is explicit.

...
class CaptureTests(unittest.TestCase):
 ...
 def test_isolation_release_and_normalisation(self):
  ...
  rows=await_messages(self.b,self.a,.1); self.assertEqual(len(rows),1); self.assertEqual(extract(rows[0],"code"),["123456"]); self.assertEqual(len(extract(rows[0],"link")),1)

...
describe("startHeartbeat wire semantics", () => {
  ...
  it("missing captureCheckIn does not consume provisioning (retries next tick)", async () => {
    const sentry = fakeSentry();
    const bare: { captureCheckIn?: typeof sentry.captureCheckIn } = {};
    const hb = startHeartbeat({ surface: "s", sentry: bare, intervalMs: 60_000 });
    bare.captureCheckIn = sentry.captureCheckIn; // SDK becomes available late
    await hb.tick();
    hb.stop();
    // the first DISPATCHED send still carries the auto-provisioning config
    expect(sentry.calls[0].monitorConfig).toBeDefined();
  });

  it("a throwing captureException never breaks the tick (fail-soft)", async () => {
    const sentry = fakeSentry();
    sentry.captureException = () => {
      throw new Error("sentry wrapper broken");
    };
    const hb = startHeartbeat({
      surface: "s",
      sentry,
    ...
    hb.stop();
    // check-in still went out with error status
    expect(sentry.calls.at(-1)?.checkIn.status).toBe("error");
  });
});
